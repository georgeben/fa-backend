 ...
 */
class CheckAuth {
  constructor({ attendeeRepository, decodeJwt, isJwtError }) {
  ...
  async getUserFromRequest(req) {
    ...
    const requestToken = authorizationHeader
      ...
      .pop()
      .trim();
    const payload = await this.decodeJwt(requestToken);

    const user = await this.attendeeRepository.findById(payload.sub);
    return user;
  }
